...
if __name__ == "__main__":
    captcha_file = "captcha.jpg"  # 你的验证码图片文件名

    print(f"提示：请确保在脚本所在目录下有一张名为 '{captcha_file}' 的验证码图片。")
    print("如果 Tesseract OCR 未安装或未在系统 PATH 中，程序可能无法运行或需要您在代码中指定其路径。")
    print("-" * 30)

    # 1. 预处理图片
    print(f"正在预处理图片: {captcha_file}...")
    processed_image = preprocess_image(captcha_file)

    if processed_image:
        print("图片预处理完成。")
        # (可选) 显示预处理后的图片，需要 matplotlib
        ...
        # 2. 识别文字
        # 如果你的验证码是中文，可以将 lang='chi_sim'
        # 前提是你已经安装了 Tesseract 的中文语言包
        print("正在识别文字...")
        recognized_code = recognize_text_from_image(processed_image, lang='eng')
        ...
        print("因图片预处理失败，无法进行识别。")

    print("-" * 30)
    print("重要提示:")
    print("1. 此脚本是一个基础示例。对于复杂的验证码（如有干扰线、字符扭曲、粘连、动态等），识别率可能很低。")
    print("2. 图像预处理步骤（如二值化阈值、滤波器选择）对识别效果至关重要，通常需要针对特定类型的验证码进行仔细调整。")
    print("3. Tesseract OCR 的识别效果也依赖于其训练数据和配置参数。")
    print("4. 请遵守相关网站的使用条款，不要将此类脚本用于非法或恶意目的。")
    print("5. 对于更复杂的干扰和粘连，可能需要使用 OpenCV 等更专业的图像处理库。")