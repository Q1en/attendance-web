...
def get_qrcode():
    ...
    try:
        ...
        base_url = "https://uis.nbu.edu.cn/authserver"
        target_service = "https://attendance.nbu.edu.cn/api/getIndex"
        login_url = f"{base_url}/login?service={target_service}"
        
        # 第一步: 访问登录页，获取关键的 'execution' 参数
        response_page = req_session.get(login_url, verify=False, timeout=15)
        response_page.raise_for_status()
        soup = BeautifulSoup(response_page.text, 'lxml')
        execution_input = soup.find('input', {'name': 'execution'})
        if not execution_input:
            return jsonify({'success': False, 'error': '未能获取execution参数'})
        execution = execution_input.get('value')
        
        # 第二步: 获取二维码的 UUID
        token_url = f"{base_url}/qrCode/getToken?ts={int(time.time()*1000)}"
        response_token = req_session.get(token_url, verify=False, timeout=10)
        response_token.raise_for_status()
        uuid = response_token.text.strip()
        if not uuid:
            return jsonify({'success': False, 'error': '未能获取二维码UUID'})
        
        # 第三步: 获取二维码图片
        qr_image_url = f"{base_url}/qrCode/getCode?uuid={uuid}"
        response_qr = req_session.get(qr_image_url, verify=False, timeout=10)
        response_qr.raise_for_status()
        
        # 将二维码图片转换为base64
        import base64
        qrcode_base64 = base64.b64encode(response_qr.content).decode('utf-8')
        ...
        return jsonify({'success': False, 'error': f'网络请求失败: {e}'})
    except Exception as e:
        return jsonify({'success': False, 'error': f'获取二维码失败: {e}'})


@app.route('/check_qrcode_status', methods=['POST'])
def check_qrcode_status():
    ...
    try:
        ...
        uuid = data.get('uuid')
        if not uuid:
            return jsonify({'success': False, 'error': '缺少UUID参数'})
        
        req_session = requests.Session()
        
        base_url = "https://uis.nbu.edu.cn/authserver"
        status_url = f"{base_url}/qrCode/getStatus.htl"
        
        params = {'ts': int(time.time()*1000), 'uuid': uuid}
        response_status = req_session.get(status_url, params=params, verify=False, timeout=10)
        status_code = response_status.text.strip()
        ...
        return jsonify({'success': False, 'error': f'检查状态失败: {e}'})
...
def handle_qrcode_login():
    ...
    try:
        ...
        if not uuid or not execution:
            flash('二维码登录参数错误。', 'error')
            return render_template('login.html')
        ...
        req_session.headers.update(base_headers)
        ...
        response_post = req_session.post(login_url, data=post_data, allow_redirects=False, verify=False, timeout=15)
        
        if response_post.status_code == 302:
            redirect_location = response_post.headers.get('Location')
            if not redirect_location:
                flash('二维码登录失败：重定向响应中缺少 Location 头。', 'error')
                return render_template('login.html')
            
            print(f"✅ [QR_LOGIN] 二维码登录成功，收到重定向: {redirect_location}")
            parsed_url = urlparse(redirect_location)
            query_params = parse_qs(parsed_url.query)
            ticket = query_params.get('ticket', [None])[0]
            
            if not ticket:
                flash('二维码登录失败：未能从重定向URL中提取 ticket。', 'error')
                return render_template('login.html')
            
            # 使用 ticket 访问目标服务并获取 JSESSIONID
            return process_login_ticket(req_session, ticket)
        else:
            flash('二维码登录失败：服务器返回意外状态。', 'error')
            return render_template('login.html')
            
    except Exception as e:
        flash(f'二维码登录过程中发生错误: {e}', 'error')
        print(f"❌ [QR_LOGIN] 二维码登录错误: {e}")
        traceback.print_exc()
        return render_template('login.html')
...
def process_login_ticket(req_session, ticket):
    """处理登录ticket并获取JSESSIONID"""
    try:
        # 使用 ticket 访问目标服务
        service_url_template = "https://attendance.nbu.edu.cn/api/getIndex"
        target_service_url = f"{service_url_template}?ticket={ticket}"
        
        base_headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36 Edg/140.0.0.0"
        }

        print(f"ℹ️ [LOGIN] 正在使用 ticket 访问目标服务: {target_service_url}")
        ...
        if jsession_id_found:
            ...
            print("❌ [LOGIN] 未能从 attendance.nbu.edu.cn 域获取到 JSESSIONID。")
            print("ℹ️ [LOGIN] 当前 req_session 中的 Cookies:")
        ...
        flash(f'访问目标服务时发生网络错误: {e}', 'error')
        print(f"❌ [LOGIN] 网络错误: {e}")
        return render_template('login.html')
    except Exception as e:
        flash(f'处理登录凭证时发生错误: {e}', 'error')
        print(f"❌ [LOGIN] 错误: {e}")
        traceback.print_exc()
        return render_template('login.html')
